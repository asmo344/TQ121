...
namespace UsbDeviceDescript
{
    ...
    public partial class USBList
    {
        ...
        /// </summary>        
        public static void SetSpecificUsbDeviceID(params (ushort VendorID, ushort ProductID)[] ID)
        {
            RegexPattern = string.Empty;
            foreach (var item in ID)
            {
                RegexPattern += $"VID_{item.VendorID:X4}&PID_{item.ProductID:X4}|";
            }
            RegexPattern = RegexPattern.Remove(RegexPattern.LastIndexOf("|"));
        }
        ...
        public static PnPEntityInfo[] AllUsbDevices
        {
            get
            {
                ...
                //return WhoUsbDevice(UInt16.MinValue, UInt16.MinValue);
            }
        }
        ...
        public static PnPEntityInfo[] WhoUsbDevice(UInt16 VendorID, UInt16 ProductID, Guid ClassGuid)
        {
            ...
            if (USBControllerDeviceCollection != null)
            {
                ...
                {   // 获取设备实体的DeviceID
                    ...
                    // 过滤掉没有VID和PID的USB设备
                    Match match;
                    if (!RegexPattern.Equals(string.Empty))
                        match = Regex.Match(Dependent, RegexPattern);
                    else
                        match = Regex.Match(Dependent, "VID_[0-9|A-F]{4}&PID_[0-9|A-F]{4}");
                    if (match.Success)
                    {
                        ...
                        if (PnPEntityCollection != null)
                        {
                            foreach (ManagementObject Entity in PnPEntityCollection)
                            {
                                ...
                                //Console.WriteLine($"USB Name: {Element.Name}");
